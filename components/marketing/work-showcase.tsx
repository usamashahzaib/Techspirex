  ...
  return (
    <section className="relative isolate overflow-hidden border-b border-[#6b56ad] bg-brand-violet-deep text-brand-paper">
      ...
      <div className="relative z-10 mx-auto max-w-[1400px] px-4 py-20 sm:px-6 lg:px-8 lg:py-32">
              ...
              <div>
                ...
                <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                  <a
                    ...
                    className="glow-signal group inline-flex min-h-12 items-center justify-center gap-2 rounded-lg bg-brand-cyan px-6 py-3 text-sm font-extrabold text-brand-ink-elevated hover:-translate-y-1"
                  >
                    Open byQalam
                  ...
                  <Link
                    ...
                    className="group inline-flex min-h-12 items-center justify-center gap-2 rounded-lg border border-white/20 px-6 py-3 text-sm font-bold transition-[transform,border-color,background-color] duration-300 hover:-translate-y-1 hover:border-brand-cyan hover:bg-white/[0.06]"
                  >
                    Read the case study
                  </Link>
                </div>
        ...
        <Eyebrow size="sm" tone="lilac" className="mt-16">
        ...
        <div className="reveal-scroll-stagger mt-5 divide-y divide-white/15 border-y border-white/15">
          {demos.map((demo) => (
            <Link
              key={demo.href}
              href={demo.href}
              className="group grid gap-4 py-8 transition-[background-color,color,padding] duration-500 ease-[var(--ease-expo-out)] hover:bg-brand-cyan hover:text-brand-violet-deep sm:grid-cols-[3rem_0.55fr_1.1fr_auto] sm:items-center sm:px-5 sm:hover:px-7"
            >
