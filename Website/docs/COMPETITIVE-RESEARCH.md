...
- Generic stock/AI-generated hero imagery or a vague abstract 3D blob
- A wall of short testimonials with first names only
...
Studied for how they build trust and communicate technical seriousness, not for their visual language:

- **Work & Co, Instrument, MetaLab, Fantasy** — build credibility primarily through named, detailed case studies with real client outcomes and visible process artifacts, not stat bands. Proof is specific (one real project explained in depth) rather than aggregate (a number with no source).
- **Linear, Vercel, Stripe** (as product companies with strong marketing sites, not as agencies) — demonstrate that restraint, a tight color and type system, and real product screenshots outperform decorative flourish. Their sites earn "premium" through precision and consistency, not ornamentation.
- **Clay, AND Digital** — full-service positioning done credibly: process is explained in operational detail (who does what, what the client provides, what they get back) rather than described as revolutionary.

## 3. What "premium" actually signals in this category (2026)

Per current design-industry consensus (award criteria increasingly weight this): premium is no longer "looks expensive" — it is the combination of a distinctive visual system, real accessibility and performance discipline, and content that reads as authored by someone with actual expertise, not templated. Judged categories now explicitly penalize sites that are visually striking but fail usability, semantics, or mobile performance. This directly supports the brief's requirement to treat accessibility and performance as engineering budgets, not decoration.
...
1. **Evidence over volume.** Almost every competitor in the direct set leads with inflated aggregate stats. A firm that visibly shows its real work in depth (even 2-3 strong case studies) instead of a stat band stands out immediately, and costs nothing to be truthful about.
2. **Process transparency as the actual differentiator**, not a claimed one. The current site already gestures at this ("Radical Transparency," client PM tool access) — this is a real, ownable position if it's true and gets demonstrated (e.g. a concrete description of what a client actually sees and receives at each stage) rather than asserted as a value-list word.
