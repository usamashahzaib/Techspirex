...
export function SocialProof() {
  ...
  return (
    <section className="border-b border-border bg-background">
      <div className="mx-auto max-w-[1440px] px-4 py-24 sm:px-6 lg:px-8 lg:py-36">
        <div className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:gap-20">
          ...
          <ol className="divide-y divide-border border-y border-border">
            {evidence.map(([title, detail], index) => (
              <li key={title} className="grid gap-3 py-7 sm:grid-cols-[3rem_0.7fr_1.3fr] sm:items-start">
        ...
        {testimonials.length > 0 && (
          <div className="bezel-shell mt-20 border border-dashed border-primary/40 bg-card p-2">
            <div className="bezel-core grid gap-8 p-5 sm:p-8 md:grid-cols-2">
              {testimonials.map((item) => (
                <blockquote key={item.quote}>
                  <CheckCircle className="size-5 text-primary" weight="light" aria-hidden="true" />
