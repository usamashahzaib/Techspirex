...
for (const route of routes) {
  test(`${route} returns 200, has no console errors, and has no horizontal overflow`, async ({
    ...
    // console.error and uncaught exceptions are different event channels - a
    // React hydration mismatch throws via the latter and does not always
    // reach the former, so this suite previously had a blind spot for it.
