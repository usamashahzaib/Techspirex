...
Dual-path conversion depending on visitor readiness:
- **Warm visitors** (already sold, just need a slot): book a discovery call directly.
- **Visitors still scoping**: submit a project brief via the contact form (project type, goal, budget, timeline), triaged by the team before a call is booked.

Both paths lead to the same human process, just entered at different points.
...
- Analytics: GA4
- Email delivery (contact form, newsletter): Resend
- Spam protection: Cloudflare Turnstile
- Hosting: Vercel
- Content: local typed MDX (insights, case studies), editable via git/PR, behind a swappable content adapter interface

## Anti-goals (carried from brief, restated as product decisions)

- No invented clients, metrics, awards, certifications, or testimonials
- No stat-band credibility (150+, 98%, 99.9%)
- No fake dashboard/status motifs (the "SYS_*" and "98 Score DEPLOYED" patterns from the old site are explicitly retired)
- No six-identical-card service grid
- No generic AI-agency visual language (see `DESIGN.md` bans)
